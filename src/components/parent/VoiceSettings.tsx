...
const VOICES: {
...
}[] = [
  {
    ...
    label: "Bobby",
    emoji: "🧒",
    character: "Joyeux & Espiègle",
    tone: "Voix enfantine, fun et expressive",
    desc: "Bobby est la voix officielle ! Pétillante, pleine d'énergie et d'enthousiasme. Il parle comme un vrai copain.",
    bestFor: "Voix par défaut — idéale pour tous les moments",
  },
  {
    type: "female",
    label: "Maman Bobby",
    ...
    desc: "Maman Bobby a une voix chaleureuse et enveloppante, comme une maman qui raconte une histoire du soir.",
  ...
  },
  {
    ...
    desc: "Le grand frère aventurier ! Il raconte tout avec passion et humour, transformant chaque échange en mini-aventure.",
  ...
  },
  {
    ...
    desc: "Papa Bobby a une voix calme qui inspire confiance. Bientôt disponible !",
    bestFor: "Bientôt disponible",
    locked: true,
  },
  {
    type: "sister",
    ...
    desc: "La grande sœur complice et décontractée. Bientôt disponible !",
    bestFor: "Bientôt disponible",
    locked: true,
  },
  {
...
const VoiceSettings = ({ settings, onUpdate, onBack, onSave, saved }: VoiceSettingsProps) => {
  ...
  return (
    <div className="p-4 space-y-4" style={{ fontFamily: "'Nunito', sans-serif" }}>
      ...
      <h2 className="text-[18px] font-black text-black uppercase tracking-wide">🎤 VOIX DE BOBBY</h2>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="text-[15px] font-black text-black uppercase">{v.label}</h3>
                    <span className="text-[9px] px-2 py-0.5 border-2 border-black font-black bg-white text-black">
                      {v.character}
                    </span>
                  </div>
                  <p className="text-[11px] text-black/70 font-bold mt-0.5">{v.tone}</p>
                  <p className="text-[10px] text-black/60 mt-1 leading-snug">{v.desc}</p>
                  <p className="text-[9px] mt-1.5 font-black text-black/80">✨ {v.bestFor}</p>
                </div>
                {!v.locked && selected && (
                  <button
                    onClick={(e) => { e.stopPropagation(); previewVoice(v.type); }}
                    disabled={!!previewPlaying}
                    className="w-10 h-10 border-4 border-black bg-white text-black flex items-center justify-center shrink-0 hover:opacity-90 transition-all"
      ...
      <div className="retro-card retro-card-tilt-2 p-4" style={{ backgroundColor: "var(--retro-yellow)" }}>
        <h3 className="text-[14px] font-black text-black mb-3 uppercase">⚡ VITESSE DE PAROLE</h3>
        <div className="grid grid-cols-3 gap-2">
          {SPEEDS.map(s => (
            <button key={s.value} onClick={() => onUpdate("voiceSpeed", s.value)}
              ...
              <span className={`text-[11px] font-black block mt-1 uppercase ${settings.voiceSpeed === s.value ? "text-background" : "text-black"}`}>{s.label}</span>
              <span className={`text-[9px] leading-tight block mt-0.5 font-bold ${settings.voiceSpeed === s.value ? "text-background/70" : "text-black/60"}`}>{s.desc}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="retro-card retro-card-tilt-5 p-4" style={{ backgroundColor: "var(--retro-purple)" }}>
        <h3 className="text-[14px] font-black text-black mb-1 uppercase">🎭 TON DE BOBBY</h3>
        <p className="text-[11px] text-black/70 mb-3 font-bold">Influence la manière dont Bobby s'exprime</p>
        <div className="grid grid-cols-2 gap-2">
          {PERSONALITIES.map(p => (
            <button key={p.value} onClick={() => onUpdate("personality", p.value)}
              ...
              <h4 className={`text-[12px] font-black mt-1 uppercase ${settings.personality === p.value ? "text-background" : "text-black"}`}>{p.label}</h4>
              <p className={`text-[9px] mt-0.5 font-bold ${settings.personality === p.value ? "text-background/70" : "text-black/60"}`}>{p.desc}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="retro-card retro-card-tilt-1 p-4" style={{ backgroundColor: "var(--retro-green)" }}>
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-[13px] font-black text-black uppercase">🔊 VOLUME DES SONS</h3>
          <span className="text-[12px] font-mono font-black text-black border-2 border-black px-2 py-0.5 bg-white">{Math.round(settings.sfxVolume * 100)}%</span>
        </div>
