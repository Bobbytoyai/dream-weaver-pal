    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-6"
      ...
      <div className="relative w-full max-w-sm overflow-hidden border-4 border-black bg-white"
        ...
        <button onClick={onCancel}
          className="absolute top-3 right-3 w-8 h-8 border-2 border-black bg-white flex items-center justify-center text-black hover:bg-muted transition-colors">
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="p-6 pt-8 text-center">
          ...
          {/* Text */}
          <h3 className="text-lg font-black text-black uppercase mb-2">{title}</h3>
          <p className="text-[13px] text-black/70 font-bold leading-relaxed mb-6">{description}</p>

          {/* Buttons */}
          <div className="flex gap-3">
            <button onClick={onCancel}
              className="flex-1 py-3 border-4 border-black bg-white text-black text-[13px] font-black uppercase hover:bg-muted transition-all"
            ...
            <button onClick={onConfirm}
              className={`flex-1 py-3 border-4 border-black text-[13px] font-black uppercase transition-all hover:opacity-90 ${
                isDanger
                  ? "bg-[var(--retro-red)] text-black"
                  : "bg-[var(--retro-yellow)] text-black"
              }`}
              style={{ boxShadow: "3px 3px 0px rgba(0,0,0,0.15)" }}>
              {confirmLabel}
            </button>
