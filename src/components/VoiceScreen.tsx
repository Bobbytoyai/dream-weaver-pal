...
const VoiceScreen = ({
  ...
  // ── Snoring sound when Bobby sleeps ──
  const snoreCtxRef = useRef<AudioContext | null>(null);
  const snoreNodesRef = useRef<{ osc: OscillatorNode; lfo: OscillatorNode; gain: GainNode } | null>(null);

  useEffect(() => {
    if (sm.machineState === "SLEEP") {
      try {
        ...
        // LFO for volume modulation (breathing rhythm)
        const lfo = ctx.createOscillator();
        lfo.type = "sine";
        lfo.frequency.value = 0.35; // ~3s cycle

        const lfoGain = ctx.createGain();
        lfoGain.gain.value = 0.015; // very soft
        ...
        masterGain.gain.linearRampToValueAtTime(0.012, ctx.currentTime + 1.5);

        snoreNodesRef.current = { osc, lfo, gain: masterGain };
      } catch (e) {
        console.warn("[Snore] Audio init failed:", e);
      }
    } else {
      // Stop snoring
      if (snoreNodesRef.current && snoreCtxRef.current) {
        const { gain } = snoreNodesRef.current;
        const ctx = snoreCtxRef.current;
        try {
          gain.gain.linearRampToValueAtTime(0, ctx.currentTime + 0.5);
          setTimeout(() => {
            try { ctx.close(); } catch {}
            snoreCtxRef.current = null;
            snoreNodesRef.current = null;
          }, 600);
        } catch {
          try { ctx.close(); } catch {}
          snoreCtxRef.current = null;
    ...
    return () => {
      if (snoreCtxRef.current) {
        try { snoreCtxRef.current.close(); } catch {}
        snoreCtxRef.current = null;
        snoreNodesRef.current = null;
      }
    };
